import { useState, useEffect } from "react";
import { GoogleMap, useJsApiLoader, Marker } from '@react-google-maps/api';
import { useLocation } from "react-router";
import './ExplorePage.css';
import '../../App.css';

export default function ExplorePage() {
    const location = useLocation();
    const place = location.state;
    const [center, setCenter] = useState({ lat: 39.9526, lng: -75.1652 }); // default Philly
    const [loadingGeocode, setLoadingGeocode] = useState(false);

    const { isLoaded, loadError } = useJsApiLoader({
        googleMapsApiKey: import.meta.env.VITE_GOOGLE_MAPS_API_KEY
    });

    useEffect(() => {
        // If lat/lng exists, use it
        if (place?.lat && place?.lng) {
            setCenter({ lat: place.lat, lng: place.lng });
        } else if (place?.name) {
            setLoadingGeocode(true);

            const geocode = async () => {
                try {
                    const response = await fetch(
                        `https://maps.googleapis.com/maps/api/geocode/json?address=${encodeURIComponent(place.name)}&key=${import.meta.env.VITE_GOOGLE_MAPS_API_KEY}`
                    );

                    const data = await response.json();

                    if (data.status === "OK") {
                        const loc = data.results[0].geometry.location;

                        setCenter({ lat: loc.lat, lng: loc.lng });
                    } else {
                        console.error("Geocoding failed:", data.status);
                    }
                } catch (err) {
                    console.error("Error fetching geocode:", err);
                } finally {
                    setLoadingGeocode(false);
                }
            };

            geocode();
        }   
    }, [place]);

    if (loadError) 
        return 
            <div>Map failed to load: {String(loadError)}</div>;
    if (!isLoaded) 
        return 
            <div>Loading map script…</div>;
    if (loadingGeocode) 
        return 
            <div>Loading location…</div>;

    return (
        <div> 
            <main className="explore-main"> 
                <div className="container"> 
                    <GoogleMap
                        mapContainerClassName="map-container"
                        center={center}
                        zoom={12}
                    > 
                        <Marker position={center} /> 
                    </GoogleMap> 

                    <div className="places-container"> 

                        <button className="up-left">▲</button> 

                        <div className="places">

                        </div>
                        
                        <button className="down-right">▼</button> 
                    </div> 
                </div> 
            </main> 
            <button id="responsiveButton">
                View my List
            </button> 
            
            <div className="list-container"> 
                <h2>List:</h2> 

                    <ul> 
                        <li>Place one</li> 
                        <li>Place two</li> 
                        <li>Place three</li> 
                    </ul>

                <div className="list-container-buttons"> 
                    <button>Save List</button> 
                    <button>Edit List</button> 
                </div> 
            </div> 
        </div>
);
}
